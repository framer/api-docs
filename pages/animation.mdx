...
- <Ref name="MotionCallbacks.onAnimationStart">
    onAnimationStart
  </Ref>
...
The `useAnimation` hook can be used to create a set of <Link href="animation/#animationcontrols">imperative `AnimationControls`</Link> with a `start` and `stop` method. These controls can be passed to **one or more** `motion` components via the `animate` prop.
...
<APIProperty name="Spring.duration" />
<APIProperty name="Spring.bounce" />
