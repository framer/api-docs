...
The focus gesture detects when a component gains or loses focus through click, focus(), or by tabIndex.
...
<APIProperty name="FocusHandlers.whileFocus" />
<APIMethod name="FocusHandlers.onFocusStart()" />
<APIMethod name="FocusHandlers.onFocusEnd()" />
...
If the draggable component is also used in <Link href="/pages/motion/animation/#layout-animations">layout animations</Link>, the drag gesture is applied to the component's viewport box instead of its x/y transforms.

```jsx
<motion.div drag layout />
```

In this model, you can still animate and adjust `x` and `y`, but they will be **additive** to the gesture.
