...
This could be useful but the strict hierarchal relationship was limiting and the use of labels created brittle soft dependencies.

Motion tracks the state and velocity of every animating value [with a `MotionValue`](/api/motion/motionvalue). Usually, it creates these by default, but by manually creating them they can be passed between components, and interesting relationships can be formed with the [`useTransform` hook](/api/motion/motionvalue#usetransform).
