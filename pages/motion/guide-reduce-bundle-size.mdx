...
For example, if import only `AnimatePresence` and `usePresence` to implement your own exit animations with CSS, you'd only add **1.4kb** to your bundle.
...
Framer Motion's simple, props-driven API is the reason for this extra size. Unused features aren't code-split, as it isn't currently possible at build time to detect which features you're using.

However, Motion offers APIs that allow you to create loading strategies so you only ever import what you need. In 2.2 this can enable you to reduce the `motion` import size down to **19kb**, but future versions will be able to reduce this even further.
...
By default, `m` has no knowledge of features like exit animations or drag. They can be loaded manually via the `MotionConfig` component. This will provide **only the features you use** to `m` components via React context.
