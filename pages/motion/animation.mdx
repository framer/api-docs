...
The `useAnimation` hook can be used to create a set of imperative `AnimationControls` LINK TO ANIMATIONCONTROLS API with a `start` and `stop` method. These controls can be passed to **one or more** `motion` components via the `animate` prop.
