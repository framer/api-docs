...
If we don't find one in `animate`, it'll check in `style`, or fallback to its initially-recorded value (for instance if it had to read one from the DOM as none was defined).

## That's all!

These are subtle changes that didn't re
