...
Framer Motion can animate the value of CSS variables, and also read CSS variables as animation targets.

#### Using pre-defined CSS variables in animation

<div>

`motion` components can animate to and from CSS variables, as long as that variable is defined on a component ancestor.
...
Also, as CSS variables are arbitrary, Framer Motion can't infer their default type. So you're able to animate `rotate` as a number because Framer Motion understands that it should be set in `deg`, whereas `'--rotate'` needs to be set as `'360deg'`.
...
Framer Motion animates values outside the React render cycle for increased performance.

Using `MotionValue`s LINK TO MOTION VALUES instead of state to update visual properties will also avoid re-renders.
