...
The `Frame` is a basic container for layout, styling, animation and events. It’s essentially a `div` element with extra options and smart defaults. Here, you see how easy it is to add visual properties, like `background` and `radius`.
...
It’s based on a `Frame`, which means it supports all Frame properties. In this example, we’re displaying a set of Frames. Notice how Stack handles the layout. It’s very easy to customize, too: try adding `direction={"horizontal"}` to change the direction, for example.
...
export function MyComponent() {
  return (
    <Stack size={150} background={"transparent"} gap={10}>
